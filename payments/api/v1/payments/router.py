...
from payments.api.v1.payments import service
...
async def get_payment(
    ...
    *,
    ...
    payment = await service.get_payment(session, payment_id=payment_id)
    if payment is None:
        raise HTTPException(
...
async def create_payment(
    *,
    ...
    payment, created = await service.create_payment(session, data=data, idempotency_key=idempotency_key)
    if not created:
